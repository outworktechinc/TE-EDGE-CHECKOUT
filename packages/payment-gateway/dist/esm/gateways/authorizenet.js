...
function getAuthNetAuthData(adapter) {
    ...
    if (!clientKey || !apiLoginID) {
        throw new PaymentError(PaymentErrorCode.CONFIG_MISSING, "Missing Authorize.Net configuration: authorizeNetClientKey and authorizeNetApiLoginId required");
    }
    return {
        clientKey,
        apiLoginID
 ...
 */
export async function initializeAuthorizeNet(adapter) {
    if (!adapter.isBrowser()) {
        throw new PaymentError(PaymentErrorCode.SDK_LOAD_FAILED, "Cannot initialize Authorize.Net in server environment");
    }
    ...
    acceptJsReadyPromise = (async () => {
        try {
            ...
            if (!window.Accept) {
                throw new PaymentError(PaymentErrorCode.SDK_LOAD_FAILED, "Accept.js loaded but window.Accept is undefined");
            }
            ...
            throw new PaymentError(PaymentErrorCode.SDK_LOAD_FAILED, "Failed to initialize Authorize.Net SDK", error);
        }
    })();
    return acceptJsReadyPromise;
}
 ...
 */
export async function createAuthorizeNetToken(card, adapter) {
    if (!acceptJsReady || !window.Accept) {
        throw new PaymentError(PaymentErrorCode.NOT_READY, "Authorize.Net SDK not initialized. Call initializeAuthorizeNet() first.");
    }
        ...
        try {
            ...
            window.Accept.dispatchData({
                ...
                if (response.messages.resultCode === "Error") {
                    ...
                    reject(new PaymentError(PaymentErrorCode.INVALID_CARD, errorMsg, { response }));
                    return;
                }
                if (!response.opaqueData?.dataValue) {
                    reject(new PaymentError(PaymentErrorCode.TOKENIZATION_FAILED, "Authorize.Net returned no opaque data value", { response }));
                    return;
                }
            ...
            reject(new PaymentError(PaymentErrorCode.TOKENIZATION_FAILED, "Failed to create Authorize.Net payment nonce", error));
        }
    });
}
/**
 ...
 */
export function resetAuthorizeNet() {
    acceptJsReady = false;
    acceptJsReadyPromise = null;
 ...
 */
export function isAuthorizeNetReady() {
