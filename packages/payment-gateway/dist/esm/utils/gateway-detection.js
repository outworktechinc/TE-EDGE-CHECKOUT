 ...
 */
export async function detectActiveGateway(adapter) {
    const apiBaseUrl = adapter.getConfig('apiBaseUrl');
    if (!apiBaseUrl) {
        throw new PaymentError(PaymentErrorCode.CONFIG_MISSING, 'apiBaseUrl is required in configuration');
    }
    try {
        ...
        if (!response.ok) {
            throw new PaymentError(PaymentErrorCode.DETECTION_FAILED, `Gateway detection failed: ${response.status} ${response.statusText}`);
        }
        const data = await response.json();
        if (!data.Status) {
            throw new PaymentError(PaymentErrorCode.DETECTION_FAILED, data.message || 'Gateway detection failed');
        }
        ...
        throw new PaymentError(PaymentErrorCode.DETECTION_FAILED, 'Failed to detect active gateway', error);
    }
}
/**
 * Determine payment scenario based on API response
 */
export function determinePaymentScenario(response) {
    ...
    throw new PaymentError(PaymentErrorCode.NOT_SUPPORTED, `Unsupported payment configuration: ${gatewayName} with ${paymentThrough}`);
}
/**
 * Check if Stripe redirect is required
 */
export function requiresStripeRedirect(config) {
    return config.scenario === 'stripe-redirect';
}
/**
 * Check if Edge Checkout is required
 */
export function requiresEdgeCheckout(config) {
    return config.scenario === 'braintree-edge' || config.scenario === 'authorizenet-edge';
}
/**
 * Get expected token type for configuration
 */
export function getExpectedTokenType(config) {
    return config.tokenType;
}
/**
 * Validate payment configuration
 */
export function validatePaymentConfiguration(config) {
    const errors = [];
