 ...
 */
export async function createStripeSession(request, adapter) {
    const apiBaseUrl = adapter.getConfig('apiBaseUrl');
    if (!apiBaseUrl) {
        throw new PaymentError(PaymentErrorCode.CONFIG_MISSING, 'apiBaseUrl is required for Stripe session creation');
    }
    try {
        ...
        if (!response.ok) {
            ...
            throw new PaymentError(PaymentErrorCode.TOKENIZATION_FAILED, errorData.message || `Failed to create Stripe session: ${response.status}`, errorData);
        }
        const data = await response.json();
        if (!data.sessionId) {
            throw new PaymentError(PaymentErrorCode.TOKENIZATION_FAILED, 'Invalid response: sessionId missing');
        }
        ...
        throw new PaymentError(PaymentErrorCode.TOKENIZATION_FAILED, 'Failed to create Stripe checkout session', error);
    }
}
/**
...
export function redirectToStripeCheckout(url) {
    if (typeof window === 'undefined') {
        throw new PaymentError(PaymentErrorCode.NOT_SUPPORTED, 'Stripe redirect is only available in browser environment');
    }
 ...
 */
export function isStripeSuccessUrl(url) {
 ...
 */
export function isStripeCancelUrl(url) {
    const targetUrl = url || (typeof window !== 'undefined' ? window.location.href : '');
    if (!targetUrl) {
        return false;
    }
 ...
 */
export async function retrieveStripeSession(sessionId, adapter) {
    const apiBaseUrl = adapter.getConfig('apiBaseUrl');
    if (!apiBaseUrl) {
        throw new PaymentError(PaymentErrorCode.CONFIG_MISSING, 'apiBaseUrl is required');
    }
    try {
        ...
        if (!response.ok) {
            throw new PaymentError(PaymentErrorCode.NETWORK_ERROR, `Failed to retrieve session: ${response.status}`);
        }
        const data = await response.json();
        ...
        throw new PaymentError(PaymentErrorCode.NETWORK_ERROR, 'Failed to retrieve Stripe session details', error);
    }
}
/**
 ...
 */
export function parseAmountFromStripe(amount, currency = 'USD') {
    const zeroDecimalCurrencies = ['BIF', 'CLP', 'DJF', 'GNF', 'JPY', 'KMF', 'KRW', 'MGA', 'PYG', 'RWF', 'UGX', 'VND', 'VUV', 'XAF', 'XOF', 'XPF'];
    if (zeroDecimalCurrencies.includes(currency.toUpperCase())) {
        return amount;
    }
