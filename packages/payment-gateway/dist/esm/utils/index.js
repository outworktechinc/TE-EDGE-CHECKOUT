 ...
 */
export function loadScript(src, adapter) {
    if (!adapter.isBrowser()) {
        return Promise.reject(new PaymentError(PaymentErrorCode.SDK_LOAD_FAILED, "Cannot load scripts in server environment"));
    }
    ...
    const loadPromise = new Promise((resolve, reject) => {
        ...
        script.onerror = () => {
            loadingScripts.delete(src);
            reject(new PaymentError(PaymentErrorCode.SDK_LOAD_FAILED, `Failed to load script: ${src}`));
        };
 ...
 */
export function normalizeGatewayName(name) {
...
export async function waitFor(condition, timeout = 10000, interval = 100) {
    ...
    while (!condition()) {
        if (Date.now() - startTime > timeout) {
            throw new PaymentError(PaymentErrorCode.SDK_LOAD_FAILED, "Timeout waiting for condition");
        }
