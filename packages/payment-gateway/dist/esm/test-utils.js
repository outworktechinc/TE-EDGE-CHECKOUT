 ...
 */
export function getTestCard(gateway, scenario = 'success') {
    const cards = {
 ...
 */
export function getAllTestCards(gateway) {
    const cards = {
        'Stripe': TEST_CARDS.stripe,
        'Braintree': TEST_CARDS.braintree,
        'Authorize.Net': TEST_CARDS.authorizenet
 ...
 */
export function createMockAdapterWithFetch(fetchMock, config = {}) {
 ...
 */
export async function failingFetch(_url, _options) {
    return new Response(JSON.stringify({
        success: false,
        error: 'Mock error'
    }), {
 ...
 */
export async function timeoutFetch(_url, _options) {
 ...
 */
export function generateRandomCard(gateway = 'Stripe') {
