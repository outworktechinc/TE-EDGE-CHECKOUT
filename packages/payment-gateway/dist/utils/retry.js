...
async function withRetry(fn, options = {}) {
    const { maxAttempts = 3, delayMs = 1000, backoffMultiplier = 2, maxDelayMs = 10000, retryableErrors = [
        types_1.PaymentErrorCode.NETWORK_ERROR,
        types_1.PaymentErrorCode.SDK_LOAD_FAILED
    ], onRetry } = options;
    let lastError;
    let delay = delayMs;
...
async function withTimeout(fn, timeoutMs, timeoutMessage = 'Operation timed out') {
    return Promise.race([
        fn(),
        new Promise((_, reject) => setTimeout(() => reject(new types_1.PaymentError(types_1.PaymentErrorCode.NETWORK_ERROR, timeoutMessage)), timeoutMs))
    ]);
}
/**
 * Retry with both timeout and exponential backoff
 */
async function withRetryAndTimeout(fn, retryOptions = {}, timeoutMs = 30000) {
