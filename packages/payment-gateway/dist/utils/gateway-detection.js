...
exports.detectActiveGateway = detectActiveGateway;
exports.determinePaymentScenario = determinePaymentScenario;
exports.requiresStripeRedirect = requiresStripeRedirect;
exports.requiresEdgeCheckout = requiresEdgeCheckout;
exports.getExpectedTokenType = getExpectedTokenType;
exports.validatePaymentConfiguration = validatePaymentConfiguration;
exports.formatConfigurationSummary = formatConfigurationSummary;
 ...
 */
async function detectActiveGateway(adapter) {
    const apiBaseUrl = adapter.getConfig('apiBaseUrl');
    if (!apiBaseUrl) {
        throw new types_1.PaymentError(types_1.PaymentErrorCode.CONFIG_MISSING, 'apiBaseUrl is required in configuration');
    }
    try {
        logger_1.logger.info('Detecting active payment gateway...', { apiBaseUrl });
        const response = await adapter.fetch(`${apiBaseUrl}/api/integration/getDefaultSubscriptionType`, {
            method: 'GET',
            headers: {
        ...
        if (!response.ok) {
            throw new types_1.PaymentError(types_1.PaymentErrorCode.DETECTION_FAILED, `Gateway detection failed: ${response.status} ${response.statusText}`);
        }
        const data = await response.json();
        if (!data.Status) {
            throw new types_1.PaymentError(types_1.PaymentErrorCode.DETECTION_FAILED, data.message || 'Gateway detection failed');
        }
        logger_1.logger.info('Gateway detected successfully', {
        ...
        logger_1.logger.error('Failed to detect active gateway', error);
        throw new types_1.PaymentError(types_1.PaymentErrorCode.DETECTION_FAILED, 'Failed to detect active gateway', error);
    }
}
/**
 * Determine payment scenario based on API response
 */
function determinePaymentScenario(response) {
    ...
    if (gatewayName === 'Stripe' &&
        ...
        logger_1.logger.debug('Scenario: Stripe session-based checkout');
        return {
            gatewayName: 'Stripe',
    ...
    if (gatewayName === 'Stripe' &&
        paymentThrough === 'Stripe' &&
        redirectUrl.isAvailable) {
        logger_1.logger.debug('Scenario: Stripe redirect checkout', { url: redirectUrl.url });
        return {
            gatewayName: 'Stripe',
            paymentMethod: 'Stripe',
    ...
    if (gatewayName === 'Braintree' &&
        ...
        logger_1.logger.debug('Scenario: Braintree Edge Checkout');
        return {
    ...
    if (gatewayName === 'Authorize.Net' &&
        paymentThrough === 'Edge Checkout') {
        logger_1.logger.debug('Scenario: Authorize.Net Edge Checkout');
        return {
    ...
    throw new types_1.PaymentError(types_1.PaymentErrorCode.NOT_SUPPORTED, `Unsupported payment configuration: ${gatewayName} with ${paymentThrough}`);
}
/**
 * Check if Stripe redirect is required
 */
function requiresStripeRedirect(config) {
    return config.scenario === 'stripe-redirect';
}
/**
 * Check if Edge Checkout is required
 */
function requiresEdgeCheckout(config) {
    return config.scenario === 'braintree-edge' || config.scenario === 'authorizenet-edge';
}
/**
 * Get expected token type for configuration
 */
function getExpectedTokenType(config) {
    return config.tokenType;
}
/**
 * Validate payment configuration
 */
function validatePaymentConfiguration(config) {
    const errors = [];
