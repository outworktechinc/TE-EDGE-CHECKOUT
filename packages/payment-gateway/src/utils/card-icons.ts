...
import { CardBrand, getCardBrandName } from './card-validation';
 ...
 */
export function getCardIcon(cardBrand: CardBrand): string {
  return CARD_ICONS[cardBrand] || CARD_ICONS.unknown;
}
...
export function getGatewayIcon(gateway: string): string {
  ...
  return (GATEWAY_ICONS as any)[lowerGateway] || '';
}
 ...
 */
export function preloadCardIcons(): void {
  if (typeof window === 'undefined') {
    return;
  }

  Object.values(CARD_ICONS).forEach(iconDataUri => {
    const img = new Image();
    img.src = iconDataUri;
  });
}
 ...
 */
export function createCardIconElement(cardBrand: CardBrand, options?: {
  width?: number;
  height?: number;
  className?: string;
  alt?: string;
  ...
  img.alt = options?.alt || getCardBrandName(cardBrand);
